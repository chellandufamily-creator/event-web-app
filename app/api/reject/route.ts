...
import { parseUploadReviewIds, rejectUploadDocuments } from "@/lib/upload-review";
...
export async function POST(request: Request) {
  ...
  try {
    const results = await rejectUploadDocuments(ids, gate);
    const okCount = results.filter((r) => r.ok).length;
    return NextResponse.json({ results, rejected: okCount });
  } catch (e) {
